    ...
    public class MerchantShipmentController : Controller
    {
        ...
        public async Task<IActionResult> ReviewShipment(CreateMerchantShipmentVM vm)
        {
            if (!ModelState.IsValid)
            {
                ...
                await RepopulateDropdownsForCreateView(vm); // Repopulate on failure
            ...
            // --- FIX: Repopulate dropdowns BEFORE passing the model to the Review view ---
            await RepopulateDropdownsForCreateView(vm);
        ...
        {
            // --- FIX: Relax the validation. Only require a merchant to be selected. ---
            if (vm == null || vm.MerchantId == 0)
            {
                 // Return zero-price details if not ready to calculate, preventing error modal.
        ...
        private async Task<(PriceDetailsVM, string)> CalculateMerchantShipmentPrice(CreateMerchantShipmentVM vm)
        {
            ...
            {
                // If there are no items, the price is zero. This is not an error.
                return (details, null);
            }
