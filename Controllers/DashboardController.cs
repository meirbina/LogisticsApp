...
namespace SMS.Controllers
{
    ...
    public class DashboardController : Controller
    {
        ...
        private async Task<DashboardVM> GetDashboardData()
        {
            ...
            // --- KPI BOX CALCULATIONS (UPDATED) ---
            var dailyRegularCount = await _context.Shipments.CountAsync(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd);
            var dailyMerchantCount = await _context.MerchantShipments.CountAsync(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd);
            var dailyGenericCount = await _context.GenericShipments.CountAsync(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd);
            
            var totalDailySalesRegular = await _context.Shipments.Where(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd).SumAsync(s => (decimal?)s.TotalCost) ?? 0;
            var totalDailySalesMerchant = await _context.MerchantShipments.Where(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd).SumAsync(s => (decimal?)s.TotalCost) ?? 0;
            var totalDailySalesGeneric = await _context.GenericShipments.Where(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd).SumAsync(s => (decimal?)s.TotalCost) ?? 0;

            int totalDailyReleased = await _context.ShipmentCollections.CountAsync(c => c.CollectionDate >= todayStart && c.CollectionDate <= todayEnd);
            
            var vm = new DashboardVM
            {
                TotalDailyShipments = dailyRegularCount + dailyMerchantCount + dailyGenericCount,
                TotalRegisteredMerchants = await _context.Merchants.CountAsync(),
                TotalActiveEmployees = await _context.Employees.CountAsync(),
                TotalDailyReleasedShipments = totalDailyReleased,
                TotalDailySales = totalDailySalesRegular + totalDailySalesMerchant + totalDailySalesGeneric
            };

            // --- CHART DATA CALCULATION (UPDATED) ---
            var employeeLocationMap = await _context.Employees
                .Include(e => e.ApplicationUser).Include(e => e.Location)
                .Where(e => e.ApplicationUser != null && e.Location != null)
                .ToDictionaryAsync(e => e.ApplicationUser.Email, e => e.Location.Name);
            ...
            var regularShipmentCreators = await _context.Shipments.Where(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd).Select(s => s.CreatedBy).ToListAsync();
            var merchantShipmentCreators = await _context.MerchantShipments.Where(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd).Select(s => s.CreatedBy).ToListAsync();
            var genericShipmentCreators = await _context.GenericShipments.Where(s => s.DateCreated >= todayStart && s.DateCreated <= todayEnd).Select(s => s.CreatedBy).ToListAsync();
            
            var allCreators = regularShipmentCreators.Concat(merchantShipmentCreators).Concat(genericShipmentCreators);
            ...
            // --- RECENT SHIPMENTS TABLE (UPDATED) ---
            var recentRegular = await _context.Shipments.OrderByDescending(s => s.DateCreated).Take(5)
                .Select(s => new UniversalShipmentRecord { WaybillNumber = s.WaybillNumber, DateCreated = s.DateCreated, SenderName = s.SenderName, ReceiverName = s.ReceiverName, Status = s.Status }).ToListAsync();
            var recentMerchant = await _context.MerchantShipments.Include(s => s.Merchant).OrderByDescending(s => s.DateCreated).Take(5)
                .Select(s => new UniversalShipmentRecord { WaybillNumber = s.WaybillNumber, DateCreated = s.DateCreated, SenderName = s.Merchant.BusinessName, ReceiverName = s.ReceiverName, Status = s.Status }).ToListAsync();
            var recentGeneric = await _context.GenericShipments.OrderByDescending(s => s.DateCreated).Take(5)
                .Select(s => new UniversalShipmentRecord { WaybillNumber = s.WaybillNumber, DateCreated = s.DateCreated, SenderName = s.SenderName, ReceiverName = s.ReceiverName, Status = s.Status }).ToListAsync();

            vm.RecentShipments = recentRegular.Concat(recentMerchant).Concat(recentGeneric)
                                    .OrderByDescending(s => s.DateCreated)
                                    .Take(5).ToList();
        ...
        public async Task<IActionResult> AllShipments(DateTime? startDate, DateTime? endDate)
        {
            ...
            var employeeLocationMap = await _context.Employees
                ...
                .Where(e => e.ApplicationUser != null).ToDictionaryAsync(e => e.ApplicationUser.Email, e => $"{e.Location.State.Name} ==> {e.Location.Name}");

            // 1. Fetch Regular Shipments
            var regularShipments = await _context.Shipments.Include(s => s.DestinationLocation.State)
                .Where(s => s.DateCreated >= start && s.DateCreated <= end).ToListAsync();
            allShipments.AddRange(regularShipments.Select(s => new ShipmentDisplayViewModel {
                Id = s.Id, ShipmentType = "Regular", WaybillNumber = s.WaybillNumber, DateCreated = s.DateCreated,
                Departure = employeeLocationMap.GetValueOrDefault(s.CreatedBy, "N/A"), SenderName = s.SenderName,
                ReceiverName = s.ReceiverName, Destination = $"{s.DestinationLocation.Name}, {s.DestinationLocation.State.Name}",
                TotalCost = s.TotalCost, Status = s.Status
            }));

            // 2. Fetch Merchant Shipments
            var merchantShipments = await _context.MerchantShipments.Include(s => s.Merchant).Include(s => s.DestinationLocation.State)
                .Where(s => s.DateCreated >= start && s.DateCreated <= end).ToListAsync();
            allShipments.AddRange(merchantShipments.Select(s => new ShipmentDisplayViewModel {
                Id = s.Id, ShipmentType = "Merchant", WaybillNumber = s.WaybillNumber, DateCreated = s.DateCreated,
                Departure = employeeLocationMap.GetValueOrDefault(s.CreatedBy, "N/A"), SenderName = s.Merchant.BusinessName,
                ReceiverName = s.ReceiverName, Destination = $"{s.DestinationLocation.Name}, {s.DestinationLocation.State.Name}",
                TotalCost = s.TotalCost, Status = s.Status
            }));

            // 3. --- NEW: Fetch Generic Shipments ---
            var genericShipments = await _context.GenericShipments
                .Include(s => s.DestinationLocation.State)
                .Where(s => s.DateCreated >= start && s.DateCreated <= end).ToListAsync();
            allShipments.AddRange(genericShipments.Select(s => new ShipmentDisplayViewModel {
                Id = s.Id, ShipmentType = "Generic", WaybillNumber = s.WaybillNumber, DateCreated = s.DateCreated,
                Departure = employeeLocationMap.GetValueOrDefault(s.CreatedBy, "N/A"),
                SenderName = s.SenderName, ReceiverName = s.ReceiverName,
                Destination = $"{s.DestinationLocation.Name}, {s.DestinationLocation.State.Name}",
                TotalCost = s.TotalCost, Status = s.Status
            }));
            ...
            {
                Shipments = allShipments.OrderByDescending(s => s.DateCreated).ToList(),
                StartDate = start, EndDate = end.Date
            };
            return View(vm);
        }
    }
