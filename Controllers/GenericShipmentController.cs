...
namespace SMS.Controllers
{
    ...
    public class GenericShipmentController : Controller
    {
        ...
        public GenericShipmentController(AppDbContext context, UserManager<ApplicationUser> userManager, ISmsService smsService)
        {
            _context = context;
            _userManager = userManager;
        ...
        public async Task<IActionResult> ConfirmShipment(string couponCode = null)
        {
            ...
            if (!string.IsNullOrEmpty(newShipment.SenderPhoneNumber))
            {
                // "Fire and forget" this task so it doesn't slow down the user's response.
                _ = _smsService.SendShipmentCreationSmsAsync(newShipment.SenderPhoneNumber, newShipment.WaybillNumber, newShipment.TotalCost);
            }
